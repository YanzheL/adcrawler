...
class AdUrlSpider(AdSpiderBase):
    ...
    DEFAULT_MAX_URL_TASKS = 20000
    DEFAULT_RECURSIVE_CHECK_DEPTH = 4
    DEFAULT_MAX_TASK_DEPTH = 100
    ...
    def make_request_from_task(self, serialized_task, **kwargs):
        ...
        if task['cur_depth'] > self.settings.get('MAX_TASK_DEPTH', self.DEFAULT_MAX_TASK_DEPTH):
            return
    ...
    def maybe_ad_img(self, img_tag, cur_depth=0):
        if cur_depth >= self.settings.get('RECURSIVE_CHECK_DEPTH', self.DEFAULT_RECURSIVE_CHECK_DEPTH):
            return False
        if self.has_ad(img_tag):
            return True
        if self.sibling_has_ad(img_tag):
            return True
        return self.maybe_ad_img(img_tag.parent, cur_depth + 1)

    def get_img_ad_url(self, img_tag, cur_depth=0):
        if cur_depth >= self.settings.get('RECURSIVE_CHECK_DEPTH', self.DEFAULT_RECURSIVE_CHECK_DEPTH):
            return
        ...
        best_a, best_href = self.get_best_a_tag(sibling_a_tags)
        if not best_href:
            return self.get_img_ad_url(img_tag.parent, cur_depth + 1)
        return best_href
